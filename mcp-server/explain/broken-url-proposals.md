...
- **Apply** on the implementing edits proposal writes the redirect (same outcome as `update_redirect`). When the accept reserved a new attached slug, that edit is the `creates_entry` packet (see attached-entry / `new_public_content` playbooks): one locale, `sections: []`, shared template untouched; redirect only after files exist. When the accept reserved an existing page, the edit does not create files — approve only adds the redirect.
- Database rows, detached pages, landings, and programs cannot be created via `creates_entry`; refuse the edits proposal; the idea can stay accepted.
